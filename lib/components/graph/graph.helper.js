...
function _validateGraphData(data) {
    ...
    if (!data.links || !data.links.length) {
        (0, _utils.throwWarning)("Graph", _err["default"].INSUFFICIENT_LINKS);
        data.links = [];
    }
            ...
            case _graph2["default"].SYMBOLS.CIRCLE: {
                ...
                var sourceNodeSize =
                    (nodes === null || nodes === void 0
                        ? void 0
                        : (_nodes$source = nodes[source.id]) === null || _nodes$source === void 0
                        ? void 0
                        : _nodes$source.size) || config.node.size; // cause this is a circle and A = pi * r^2
                var targetNodeSize =
                    (nodes === null || nodes === void 0
                        ? void 0
                        : (_nodes$target = nodes[target.id]) === null || _nodes$target === void 0
                        ? void 0
                        : _nodes$target.size) || config.node.size; // cause this is a circle and A = pi * r^2

                const sourceRelativePart = sourceNodeSize / config.node.size
                const targetRelativePart = targetNodeSize / config.node.size

                const sourceRadius = sourceNodeSize / 20
                const targetRadius = targetNodeSize / 20
                x1 += sourceRadius * directionVector.x;
                y1 += sourceRadius * directionVector.y; // Points from the target, we move the by the size of the radius of the circle + the size of the arrow
